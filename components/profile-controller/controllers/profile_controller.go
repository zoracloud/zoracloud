...
import (
	...
	profilev1 "github.com/zoracloud/zoracloud/components/profile-controller/api/v1"
...
// zoracloud default role names
// TODO: Make zoracloud roles configurable (krishnadurai)
// This will enable customization of roles.
const (
	zoracloudAdmin       = "zoracloud-admin"
	zoracloudEdit        = "zoracloud-edit"
	zoracloudView        = "zoracloud-view"
...
func (r *ProfileReconciler) Reconcile(request ctrl.Request) (ctrl.Result, error) {
	...
	defaultZoracloudNamespaceLabels := r.readDefaultLabelsFromFile(r.DefaultNamespaceLabelsPath)
	...
	setNamespaceLabels(ns, defaultZoracloudNamespaceLabels)
	...
	if err != nil {
		...
		if ok && owner == instance.Spec.Owner.Name {
			...
			setNamespaceLabels(foundNs, defaultZoracloudNamespaceLabels)
	...
	// "default-editor" would have zoracloudEdit permission: edit all resources in target namespace except rbac.
	if err = r.updateServiceAccount(instance, DEFAULT_EDITOR, zoracloudEdit); err != nil {
	...
	if err = r.updateServiceAccount(instance, DEFAULT_VIEWER, zoracloudView); err != nil {
		logger.Error(err, "error Updating ServiceAccount", "namespace", instance.Name, "name",
			"defaultViewer")
		IncRequestErrorCounter("error updating ServiceAccount", SEVERITY_MAJOR)
		return reconcile.Result{}, err
